...
/*
 This script will only run for 24 hours. That is so that each time it runs
 it fetches a new list of aircraft. To keep it running type `crontab -e` and
 then add the line below to the end of the file (the * are supposed to be there).
...
//send a position for each aircraft every ... seconds
$position_interval = 30; //seconds

//the stream of decoded messages coming from DUMP1090, this defaults to port 30003
...
#source of ellipsoid axis dimensions a,b : http://www.osni.gov.uk/technical/grid.doc
    ...
    function irishgrid_to_gridref($e,$n) {
        $codes = array(
            array ('V', 'W', 'X', 'Y', 'Z'),
            array ('Q','R','S','T','U'),
            array ('L','M','N','O','P'),
